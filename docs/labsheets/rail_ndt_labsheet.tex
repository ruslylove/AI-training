...
\begin{itemize}
  \item \texttt{results/figures/rail\_eval.png} -- several test photos side
  by side, each with the true and predicted defect areas overlaid directly
  on the rail photo in color -- \textbf{yellow} = correctly caught,
  \textbf{green} = a real defect the model missed, \textbf{red} = a false
  alarm -- with that photo's own IoU score labeled above it. Look at it --
  this is the most useful way to understand what your model is actually
  doing.
  \item \texttt{results/figures/rail\_eval\_threshold\_curve.png} -- Mean IoU
  and Mean Dice swept automatically across every threshold from 0.05 to
  0.95 for this same trained model, with your current threshold marked. It's
  a preview of the trade-off you'll explore by hand in Part 3.
\end{itemize}
...
\section*{Part 5 (optional): Generate a shareable report}
...
\texttt{results/reports/rail\_report.pdf} -- your settings, score, example
predictions, and the threshold curve on a single page. Re-run it any time
after Part 2 (or Part 3's threshold tweaks) to capture your current best
result.
